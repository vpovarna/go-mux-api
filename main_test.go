...
func TestNonExistingProduct(t *testing.T) {
	...
	if m["error"] != "Product not found" {
		t.Errorf("Expecter the 'error' key of the response to be set 'Product not found'. Got: %s", m["error"])
	}

}

func TestCreateAProduct(t *testing.T) {
	clearTable()
...
func checkResponseCode(t *testing.T, expectedResponseConde, actualResponseCode int) {
...
func addProducts(count int) {
	if count < 1 {
		log.Print("Receivend an invalid counter, setting up the default value: 1")
		count = 1
	}
